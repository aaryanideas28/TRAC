...
import { Sparkles, ArrowRight, Clock, Zap, GitCommit, CornerDownRight, Gauge, Layers, Filter, AlertTriangle, CheckCircle2 } from 'lucide-react';

interface AiRecommendationPanelProps {
  recommendations: AiRecommendation[];
  conflicts?: ConflictItem[];
  onAcceptRecommendation?: (recId: string) => Promise<void> | void;
}
...
export const AiRecommendationPanel: React.FC<AiRecommendationPanelProps> = ({
  ...
  const handleAccept = async (recId: string) => {
    setIsAccepting(recId);
    setAcceptedIds((prev) => new Set(prev).add(recId));
    if (onAcceptRecommendation) {
      await onAcceptRecommendation(recId);
    }
    setTimeout(() => setIsAccepting(null), 500);
  };
  ...
  const handleAcceptAll = async () => {
    ...
    if (onAcceptRecommendation) {
      await onAcceptRecommendation('ALL');
    }
    setTimeout(() => setIsAccepting(null), 500);
  };
    ...
    <div className="w-full p-5 rounded-2xl bg-slate-900/95 border border-slate-800 shadow-2xl space-y-4">
        ...
        <div className="flex items-center gap-2">
          {recommendations.length > 0 && (
            <button
              ...
              disabled={isAccepting === 'ALL'}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 text-white font-extrabold text-xs shadow-lg transition-all active:scale-95 disabled:opacity-50"
              title="Accept all AI recommendations and dispatch trains across network"
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span>{isAccepting === 'ALL' ? 'Dispatching All...' : `Accept All (${recommendations.length})`}</span>
            </button>
          )}
          ...
          {activeConflicts.length > 0 ? (
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold">
              ...
              <span>No active conflicts • 18 Trains Monitored</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filteredRecs.map((rec) => {
          const isAccepted = rec.status === 'ACCEPTED' || acceptedIds.has(rec.id);
          return (
            <div
              key={rec.id}
              className={`p-4 rounded-xl border transition shadow-lg flex flex-col justify-between space-y-3 ${
                isAccepted
            ...
            >
              ...
              {/* Card Footer: Delay Saved & Action Button */}
              <div className="pt-2.5 text-xs border-t border-slate-800/80 flex items-center justify-between gap-2">
                <div className="text-slate-400 font-mono">
                  Delay Saved: <span className="font-bold text-emerald-400">+{rec.expected_delay_reduction_min}m</span>
                </div>

                {/* Dynamic Accept & Dispatch Button */}
                {isAccepted ? (
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-950/90 border border-emerald-500/60 text-emerald-300 text-xs font-extrabold shadow-sm">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-extrabold text-xs shadow-md transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
                    title="Accept this AI recommendation and update train dispatch dynamically"
                  >
                    <Zap className="w-3.5 h-3.5 fill-current text-amber-300" />
                    <span>{isAccepting === rec.id ? 'Dispatching...' : 'Accept'}</span>
                  </button>
                )}
              </div>
            </div>
